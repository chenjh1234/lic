...
void  LLicEncrypt::setLicFile(LLicFile *p)
{
...
int LLicEncrypt::createKey()
{
    ...
    QString sPub, vPri, vSeed, venderKey, str;
   
// vender:
    infoV = lfile->vender();
    vPri= infoV->get(VENDERPRI).toString();
    vSeed= infoV->get(VENDERSEED).toString();
// seed:
    if(vSeed.length() == 0)
    {
        _err += "Err: VenderSeed length() == 0 \n";
    }

 // is serverPub,venserPri file exist:
     if (!fd.isFile(vPri))
     {
        _err += "Err: No vender.pri file found\n";
     }
 ...
 // serverID check:
    ...
    {
        _err += "Err: ServerID error  length error \n";
    }

 //get venderKey
    str =  encodeVenderKey(vPri, infoV, vSeed);
    infoV->set(VENDERKEY, str);

    if (str.length() <= 0)
    {
       _err += "Err: make VenderKey Error \n";
    }
    venderKey = str;
 // get package keys:
    int num;
    num  =  lfile->size();
    //qDebug() << "packages = "<< num;
    QString cur,start,end,pname,plimit;
    int limit;
    cur = fd.curD();
    QStringList slist;
...
// checkPackageName:
      pname = infoP->get(PACKAGENAME).toString();
      if (slist.contains(pname))   _err += "Err: Packagename : the same name error " + pname + "\n";
      slist << pname; 
// check limit:
...
// checkDate:
      ...
      //err:== start > end  ||  cur > end;
      if (start.length() !=8 || start.length() !=8)  _err += "Err: Date format error\n";
      if (!fd.isDate(start) || !fd.isDate(end))  _err += "Err: Date format error\n";

      if (start > end  || cur > end)  _err += "Err: startDate > endDate\n";
//checkType:     
       str = infoP->get(PTYPE).toString();
       if (!(str ==PTYPE_TASK || str == PTYPE_USER || str ==PTYPE_NODE)) 
       {
          _err += "Err:type error: task||node||user\n";
       }
// makekey:
       str =  digestPackageKey(venderKey, infoP);
        infoP->set(PKEY, str);
       //qDebug() << "pkey1 = " << str.length() << str << PKEY;

    }
     if (_err .length() >0)  return -1;
       else return 1;
