...
int SLicMng::loginApp(SAppInfo& msg)
{
    ...
    else
    {
       QString  appname = msg.get(APP_NAME).toString();
        // config limited:
        err = getPackID(msg) +" --- " +appname + "_" + getAppID(msg) + "--- config limited error";
        data->elog(err);
        return i;
    }
...
QString  SLicMng::getAppID(SAppInfo &msg)
{
   SAppInfo *info;

    QString pid,package,version,ip,appid;
    int number,i;
    info = &msg;
...
bool SLicMng::isLogoutConfig(SAppInfo &msg)
{
         SAppInfo *info;
      QString vender,package,version,ip,user,packid,node,licproject;
      int number,i;
...
int SLicMng::checkLic(QString filename, int mode)
{
   ...
   for (i = 0; i < num; i++)
   {
      ...
      if (str < infoP->get(PSTARTDATE).toString() || str > infoP->get(PENDDATE).toString()) 
           return  LOADFILE_DATE_ERR;
   }
...
int SLicMng::unloadFileStr(QString str, QString& proofStr)
{
   ...
   if (i < 0)
   {
      err = "unload file error: " + mapLoadFile[i] ;
      data->plog(err);
   }
   return i;
...
int SLicMng::clearConfig(QString pass)
{
    LFileDate fd;
    int iret,i;

    if (fd.isPasswdToday(pass,"clearconfig"))  
    {
            i = data->_config.clear();
            if (i >=0)  iret = 0;
            else iret = -2;
    }
    else 
