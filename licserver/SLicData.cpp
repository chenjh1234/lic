...
QString  SLicData::unloadPackage(SPackInfo &info,int mode)// called from unpack:SlicMng
{
    #if 1
...
int SLicData::removePackage(QString packid, SPackInfo *info) // base remove
{
   ...
   if (i >= 0)  i = saveDBPackage();
   qDebug() << "saveDBPack ok after remove package" << i ;
   return i; // index of info in mng <0 err
}
      ...
      if (mng != NULL)
      {
         ...
         for (i = 0; i < mng->size(); i++)
         {
            ...
            if (limit.toInt() == (-1 * info->limit) && info->isBorrowOut())
            {
               ...
               if (ty == ty1 && start == start1 && end == end1 && sinID == sinID1 && soutID == soutID1 && uuid == uuid1 && sign == sign1)
               {
                  ret = 1;
                  retInfo = info;
                  break;
...
// packid,appid
  ...
  // info->appid = encodeAppId(ip, pid);
   ...
   i = -1;
   rmInfo = NULL;
   if (mapApp.contains(info->packid) )
   {
...
// dbMsge:
   QString fpath, fcdate, fmds5, finode, fsz,ptrinode,ptrcdate;
   QFileInfo fi(fileDB);
//qDebug() << " save DBMsg";
   fpath = fi.absoluteFilePath();
   fmds5 = _dt.mds5(fileDB);       // chang the TID of the file
   ...
   qDebug() << fsz;
#endif
// make sure ptr is exist:
   if (!QFile::exists(filePtr)) 
   {
       QFile::copy(fileDB,filePtr);
   }

   ptrinode = _dt.inodeFull(filePtr);
   //ptrcdate = _dt.TID(filePtr);
   ptrcdate = _dt.curDT();
   qDebug() << " inner date  of ptr = " << ptrcdate;
   //open:
//save to PTR:
   ...
   //msg of ptr:
   ds << fpath;
   //qDebug() << "saveDBMsg----, ptr inode = " << finode;
   ds << ptrinode;
   QString randT;
   randT = _dt.randText10();
...
//open ptr:
   ...
   if (str != fmds5) RET("DB fileContains error");
   ds >> str;
   qDebug() << " DB inode = " <<str << finode;
   if (str != finode) RET("DB fileInode error1");
   ds >> str;
   qDebug() << " DB fsz= " << str << fsz ;
   if (str != fsz) RET("DB fileSize error");
qDebug() << " DB fsz= ok" ;
//======================================
   QFileInfo fii(filePtr);
...
qDebug() << " DB fsz= ok"  ;
   ...
   if (str != fpath) RET("DB_Ptr filePath error");
   ds >> str;
   qDebug() << " DB_PTR inode = " << str << finode <<str.toInt(&b,16) << finode.toInt(&b,16);
   if (str != finode) RET("DB_Ptr fileInode error");
   ds >> str; // randText10: make the ptr file rand length;
   ds >> str;
   qDebug() << " DB fsz= ok"  ;
...
int SLicData::checkPackExp(QString packid)
{
   ...
   qDebug() << "check packid Exp size =" << packid << sz;
   for (i = 0; i < sz; i++)
   {
      info = (SPackInfo *)mng->get(i);
...
int SLicData::checkPackExp()
{
   ...
   packs = packNames();
   sz = packs.size();
    qDebug() << "check pack Exp size =" <<sz;
   for (i = 0; i < sz; i++)
   {
      pack = packs[i];
      ir = checkPackExp(pack);
      qDebug() << "check ok i = " << i << ir;
      ic = ic + ir;
   }
   return ic;
}
...
QString SLicData::dbConfigName()
{
   QString s;
   s = dbDir() + "/" + DB_CONFIGFILE;
   return s;
}
...
QString  SLicData::getDBConfigFile()
{
   return dbConfigName();
}
